...
class Advanced_Categories_Widget_Views
{
	...
	public static function start_list_item( $term, $instance, $categories, $echo = true )
	{
		...
		$class      = 'acatw-list-item cat-post-item ' . $item_class;
	...
	public static function list_item( $term, $instance, $categories, $echo = true )
	{
		...
		?>
			<div id="term-<?php echo $item_id ;?>" class="<?php echo $item_class ;?> joomir-cat" >

				<?php do_action( 'acatw_item_top', $term, $instance ); ?>

					<div class="term-header acatw-term-header cat-post-thumbnail">
					...
					<?php  if( $instance['show_desc'] ) { ?>
						<span class="term-summary acatw-term-summary joomir-desc">
						<?php 
						printf( '<h3 class="term-title acatw-term-title"><a href="%s" rel="bookmark" class="post-title">%s</a></h3>',
							esc_url( get_term_link( $term ) ),
							sprintf( __( '%s', 'advanced-categories-widget'), $term->name )
						);
						
						?>
							<br><p><?php echo $item_desc; 
							$lang = get_locale(); if($lang == "fa_IR"){
						printf( '<a href="%s" class="cat-post-excerpt-more">مطالعه بیشتر</a>',
							esc_url( get_term_link( $term ) )
						);}
						else{
						printf( '<a href="%s" class="cat-post-excerpt-more">read more</a>',
							esc_url( get_term_link( $term ) )
						);
						}
						?></p>
